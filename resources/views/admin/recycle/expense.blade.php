              ...
              <tbody>
                @foreach ($all as $data)
                    <tr>
                        ...
                        <td>{{ $data->categoryExp->expcate_name }}</td>
                        <td>{{ $data->expense_amount }}</td>
                        <td>{{ $data->expense_date }}</td>
                        <td>
                            <a href="#" id="restore" data-bs-toggle="modal" data-bs-target="#restoreModal" data-id="{{ $data->expense_id }}"><i class="fas fa-solid fa-recycle fs-5 text-success fw-bold mx-1"></i></a>
...
<div class="modal fade" id="restoreModal" tabindex="-1" aria-labelledby="restoreModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <form action="{{ url('dashboard/expense/restore') }}" method="POST">
            @csrf
...
<div class="modal fade" id="deleteModal" tabindex="-1" aria-labelledby="deleteModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <form action="{{ url('dashboard/expense/delete') }}" method="POST">
            @csrf
            <div class="modal-content modal_content">
                <div class="modal-header modal_header">
